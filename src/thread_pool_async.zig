// 
// TAKEN FROM - https://github.com/kprotty/zap/blob/blog/src/thread_pool.zig
//
...
pub fn ReturnTypeOf(comptime asyncFn: anytype) type {
    return @typeInfo(@TypeOf(asyncFn)).Fn.return_type orelse type;
}
...
pub fn run(comptime asyncFn: anytype, args: anytype) ReturnTypeOf(asyncFn) {
    ...
    const num_threads = std.math.cast(u16, num_cpus) orelse std.math.maxInt(u16);
